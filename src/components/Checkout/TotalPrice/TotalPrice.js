...
const TotalPrice = () => {
  ...
  const handleToken = (token, addresses) => {
    ...
    alert(
      "Cannot recognize your payment info. Please enter correct data to proceed further."
    );
  };
  ...
  return (
    <div className="totalPrice">
      <CurrencyFormat
        renderText={(value) => (
          <>
            <span className="totalPrice__items">
              Total number of Items in your cart:{" "}
              <strong>{cart?.length}</strong>
            </span>
            <span className="totalPrice__total">
              Total Price of your items: <strong>{`${value}`}</strong>
            </span>
          </>
        )}
        decimalScale={2}
